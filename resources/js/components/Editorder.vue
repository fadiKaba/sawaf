<template>
    <div>
        ...
        <div class="mb-3 ">
            ...
            <input v-model="date" required name="orderdate" type="date" class="form-control" id="order-date" placeholder="Date">
        </div>
        <div>
            <label class="form-label">Customer</label>
            <!-- <select v-model="customerId" name="customerid" class="form-select form-select-lg form-control mb-3" aria-label=".form-select-lg example">
                <option value="1" disabled selected>{{orderu.customer.FirstName}} {{orderu.customer.LastName}}</option>            
            </select> -->
            <p class="font-weight-bold">{{orderu.customer.FirstName}} {{orderu.customer.LastName}}</p>
        </div>
        <div>
...
export default {
    name:'Editorder',
    props:['customers', 'products', 'orderu', 'orderproducts'],
    data:function(){
        return{
            orderNumber:'',
            customerId:this.orderu.CustomerId,
            productId:'',
            date:this.orderu.OrderDate.substr(0,10),
            quantity:1,
            orders:[],
            errMessage:'',
    ...
    methods:{
        addItem:function(){      
            this.orderproducts.forEach(item => {
                let ord = this.makeOrder(
                    this.orderu.OrderNumber,
                    this.orderu.CustomerId,
                    item.pivot.ProductId,
                    item.ProductName,
                    item.pivot.Quantity,
                    this.orderu.OrderDate
                    );
        ...
        makeOrder:function(orderNumber,customerId,ProductId,ProductName,ProductQuantity,  OrderDate){
          let order = {
        ...
        saveOrder:function(){
        axios.patch(`/updateorder/${this.orderu.id}`,this.orders)
              .then(response => {
